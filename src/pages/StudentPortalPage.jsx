...
export default function StudentPortalPage() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await consultarEstadoAlumno(catedraId, clean);
      if (!res.success) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-canvas text-text-primary antialiased selection:bg-primary/20 selection:text-primary">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-6">
        {/* Cabecera Bento de la Cátedra e Institución */}
        <div className="backdrop-blur-xl bg-white/75 dark:bg-slate-900/60 rounded-3xl border border-slate-200/80 dark:border-white/10 p-5 sm:p-7 shadow-xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1.5">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-[11px] text-text-muted flex items-center gap-1 font-medium bg-slate-100/60 dark:bg-white/[0.04] px-2.5 py-0.5 rounded-lg border border-slate-200/60 dark:border-white/5">
                  <Building className="w-3 h-3 text-primary" />
                  <span>{portalConfig.institucion_nombre}</span>
                </span>
              </div>
            ...
            <div className="shrink-0 flex items-center gap-2 text-xs text-text-muted bg-slate-100/60 dark:bg-white/[0.04] px-3 py-2 rounded-2xl border border-slate-200/60 dark:border-white/5">
        ...
        {!resultado ? (
          ...
          <div className="space-y-6 animate-fadeInUp">
            {/* Barra de Navegación de Consulta / Reset */}
            <div className="flex items-center justify-between flex-wrap gap-3">
              <button
                onClick={handleResetSearch}
                className="inline-flex items-center gap-1.5 text-xs font-semibold text-text-muted hover:text-primary transition-colors group cursor-pointer"
              >
                <ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
                <span>Consultar otro DNI</span>
              </button>

              <div className="flex items-center gap-2 text-xs font-mono text-text-muted bg-slate-100/80 dark:bg-white/[0.04] px-3 py-1 rounded-xl border border-slate-200/60 dark:border-white/5">
                <span>DNI Consultado:</span>
                <strong className="text-text-primary">{formatDniDisplay(resultado.estudiante?.dni)}</strong>
              </div>
            </div>
